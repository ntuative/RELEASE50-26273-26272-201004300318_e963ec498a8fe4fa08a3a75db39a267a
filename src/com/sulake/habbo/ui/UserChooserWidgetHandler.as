package com.sulake.habbo.ui
{
   ...
   public class UserChooserWidgetHandler implements IRoomWidgetHandler
   {
      ...
      {
         return RoomWidgetEnum.const_276;
      }
      ...
      private function handleUserChooserRequest() : void
      {
         ...
         var _loc3_:* = [];
         var _loc5_:int = _container.roomEngine.getRoomObjectCount(_loc1_,_loc2_,RoomObjectCategoryEnum.const_39);
         var _loc6_:int = 0;
         while(_loc6_ < _loc5_)
         {
            _loc4_ = _container.roomEngine.getRoomObjectWithIndex(_loc1_,_loc2_,_loc6_,RoomObjectCategoryEnum.const_39);
            _loc7_ = _container.roomSession.userDataManager.getUserDataByIndex(_loc4_.getId());
            if(_loc7_ != null)
            {
               _loc3_.push(new ChooserItem(_loc7_.id,RoomObjectCategoryEnum.const_39,_loc7_.name));
            }
            _loc6_++;
         }
         _loc3_.sort(compareItems);
         _container.events.dispatchEvent(new RoomWidgetChooserContentEvent(RoomWidgetChooserContentEvent.const_656,_loc3_));
      }
