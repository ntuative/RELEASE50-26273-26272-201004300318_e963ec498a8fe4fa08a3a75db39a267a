package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      private var var_1716:MarketplaceItemStats;
      ...
      {
         if(!var_151 || !_catalog || !_catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = var_151.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         showConfirmation(const_1634,_loc2_);
      }
      ...
      {
         if(param1.furniCategoryId != var_1635 || param1.furniTypeId != var_1638)
         {
            return;
         }
         var_1716 = param1;
      ...
      public function onCancelResult(param1:IMessageEvent) : void
      {
         ...
         if(parser.result == 1)
         {
            item = var_190.remove(parser.offerId);
            if(item != null)
            {
               item.dispose();
            }
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            item = var_151.remove(parser.requestedOfferId);
            if(item != null)
            {
               item.dispose();
            }
         ...
         {
            updateItem = var_151.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               var_151.add(parser.offerId,updateItem);
            }
            var_151.remove(parser.requestedOfferId);
            showConfirmation(const_1593,updateItem);
            _visualization.listUpdatedNotify();
         }
      ...
      public function redeemSoldOffers() : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         var _loc1_:Array = var_190.getKeys();
         for each(_loc2_ in _loc1_)
         {
            _loc3_ = var_190.getValue(_loc2_);
            if(_loc3_.status == MarketPlaceOfferState.const_364)
            {
               var_190.remove(_loc2_);
               _loc3_.dispose();
            }
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         var_1714 = _loc3_.creditsWaiting;
         var_190 = new Map();
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_190.add(_loc4_.offerId,_loc6_);
         }
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            _loc6_.timeLeftMinutes = _loc4_.timeLeftMinutes;
            var_151.add(_loc4_.offerId,_loc6_);
         }
         if(_visualization == null)
         {
            return;
      ...
      {
         requestOffers(var_1713,var_1717,var_1663,var_1715);
      }
