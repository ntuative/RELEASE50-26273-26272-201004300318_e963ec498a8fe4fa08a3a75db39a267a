   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!_disposed)
         {
            ...
            if(_result == const_801)
            {
               if(var_770 != null)
               {
                  Core.crash("Error catched in EventDispatcher: undefined",var_770.errorID,var_770);
               }
               else
               {
                  Core.crash("Error catched in EventDispatcher. No error data available!",0,var_770);
               }
            }
      ...
      public function removeEventListener(param1:String, param2:Function, param3:Boolean = false) : void
      {
         ...
         if(!_disposed)
         {
            _loc4_ = var_289[param1];
            if(_loc4_)
            {
               _loc5_ = 0;
               for each(_loc6_ in _loc4_)
               {
                  if(_loc6_.callback == param2 && _loc6_.var_2281 == param3)
                  {
      ...
      public function addEventListener(param1:String, param2:Function, param3:Boolean = false, param4:int = 0, param5:Boolean = false) : void
      {
         ...
         else
         {
            ...
            {
               if(_loc8_.callback == param2 && _loc8_.var_2281 == param3)
               {
                  return;
               }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         var array:Array = var_289[event.type];
         if(array)
         {
            callbacks = [];
         ...
         _result = !!event.isDefaultPrevented() ? uint(const_1138) : uint(const_507);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            ...
            {
               _loc2_ = var_289[_loc1_] as Array;
