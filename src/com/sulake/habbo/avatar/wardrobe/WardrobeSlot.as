package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit
   {
      ...
      private var var_305:IBitmapWrapperWindow;
      ...
      private function createView() : void
      {
         ...
         _view.visible = false;
         var_305 = _view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(var_316 && var_1226)
         {
            _loc5_ = var_29.avatarRenderManager.createAvatarImage(figure,AvatarScaleType.SMALL,var_579);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_40,parseInt(FigureData.const_540));
         ...
         if(var_305)
         {
            if(false)
            {
               var_305.bitmap.dispose();
            }
            var_305.bitmap = new BitmapData(var_305.width,var_305.height,true,0);
            _loc7_ = (0 - _loc1_.width) / 2;
            _loc8_ = (0 - _loc1_.height) / 2;
            var_305.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               var_316 = var_29.figureData.getFigureString();
               var_579 = var_29.gender;
               var_29.handler.saveWardrobeOutfit(var_1502,this);
               updateView();
               break;
            case "get_button":
               if(var_316)
               {
                  var_29.loadAvatarInEditor(var_316,var_579,var_29.clubMemberLevel);
               }
         }
      }
   }
