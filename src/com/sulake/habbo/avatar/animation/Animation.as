package com.sulake.habbo.avatar.animation
{
   ...
   public class Animation implements IAnimation
   {
      ...
      private var var_1599:AvatarDataContainer;
      
      private var _frames:Array;
      
      private var var_1601:DirectionDataContainer;
      
      private var var_1600:ShadowDataContainer;
      ...
      public function Animation(param1:AvatarStructure, param2:XML)
      {
         ...
         if(param2.hasOwnProperty("sprite"))
         {
            ...
            {
               var_1346.push(new SpriteDataContainer(this as IAnimation,_loc5_));
            }
         }
         if(param2.hasOwnProperty("avatar"))
         {
            var_1599 = new AvatarDataContainer(param2.avatar[0]);
         }
         if(param2.hasOwnProperty("canvas"))
         {
            for each(_loc6_ in param2.canvas)
            {
               var_1345.push(new CanvasDataContainer(_loc6_));
            }
         }
         if(param2.hasOwnProperty("direction"))
         {
            var_1601 = new DirectionDataContainer(param2.direction[0]);
         }
         if(param2.hasOwnProperty("shadow"))
         {
            var_1600 = new ShadowDataContainer(param2.shadow[0]);
         }
         if(param2.hasOwnProperty("remove"))
         {
            for each(_loc7_ in param2.remove)
            {
               var_1347.push(String(_loc7_.@id));
            }
         }
         if(param2.hasOwnProperty("add"))
         {
            ...
            {
               var_750.push(new AddDataContainer(_loc8_));
            }
         }
         var _loc3_:int = 0;
         for each(_loc4_ in param2.frame)
         {
            ...
            for each(_loc11_ in _loc4_.bodypart)
            {
               ...
               _loc13_ = new AnimationLayerData(param1,_loc11_,AnimationLayerData.const_638,_loc3_,_loc10_);
               _loc9_.push(_loc13_);
            }
            for each(_loc12_ in _loc4_.fx)
            {
               _loc10_ = param1.getActionDefinition(String(_loc12_.@action));
               _loc14_ = new AnimationLayerData(param1,_loc12_,AnimationLayerData.const_378,_loc3_,_loc10_);
               _loc9_.push(_loc14_);
               if(_loc10_ != null)
               {
               }
      ...
      {
         return var_1601 != null;
      }
      
      public function hasSpriteData() : Boolean
      {
         return var_1346 != null;
      }
      ...
      public function getLayerData(param1:int, param2:String) : AnimationLayerData
      {
         ...
         for each(_loc3_ in getFrame(param1))
         {
            ...
            if(_loc3_.type == AnimationLayerData.const_378)
            {
               for each(_loc4_ in var_750)
               {
      ...
      public function getAnimatedBodyPartIds(param1:int) : Array
      {
         ...
         for each(_loc3_ in getFrame(param1))
         {
            if(_loc3_.type == AnimationLayerData.const_638)
            {
