package com.sulake.core.window.utils
{
   ...
   public class WindowParser implements IWindowParser
   {
      ...
      public static const const_1529:String = "_INCLUDE";
      
      public static const const_998:String = "_EXCLUDE";
      
      public static const const_1623:String = "_TEMP";
      
      private static const const_1102:RegExp = /^(\s|\n|\r|\t|\v)*/m;
      
      private static const const_1103:RegExp = /(\s|\n|\r|\t|\v)*$/;
      ...
      protected var var_1400:Map;
      
      protected var var_16:IWindowContext;
      
      protected var var_1401:Dictionary;
      ...
      public function WindowParser(param1:IWindowContext)
      {
         ...
         TypeCodeTable.fillTables(var_1124,var_1123);
         var_958 = new Dictionary();
         var_1401 = new Dictionary();
         ParamCodeTable.fillTables(var_958,var_1401);
         var_1400 = new Map();
      }
      
      private static function trimWhiteSpace(param1:String) : String
      {
         param1 = param1.replace(const_1103,"");
         return param1.replace(const_1102,"");
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            ...
            for(_loc1_ in var_1401)
            {
               delete var_1401[_loc1_];
            }
            var_1400.dispose();
            var_1400 = null;
            var_16 = null;
      ...
      private function parseSingleWindowEntity(param1:XML, param2:WindowController, param3:Map) : IWindow
      {
         ...
         visible = parseAttribute(xml,"visible",variables,visible.toString()) == "HabboHelpCom_back_hi_png";
         ...
         background = parseAttribute(xml,"background",variables,window.background.toString()) == "HabboHelpCom_back_hi_png";
         blend = Number(parseAttribute(xml,"blend",variables,window.blend.toString()));
         clipping = parseAttribute(xml,"clipping",variables,window.clipping.toString()) == "HabboHelpCom_back_hi_png";
            ...
            if(parent != null)
            {
               if(parent is IIterable)
               {
                  if(window.x != rect.x || window.y != rect.y || window.width != rect.width || window.height != rect.height)
                  {
                     ...
                     if((params & 0) == WindowParam.const_179)
                     {
                        window.y = rect.y;
                     }
                  }
         ...
         if(_loc7_ is IIterable)
         {
            ...
            if(_loc9_ > 0)
            {
               ...
               while(_loc12_ < _loc9_)
               {
                  ...
                  if(_loc10_.tags.indexOf(WindowParser.const_998) == -1)
                  {
